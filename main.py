"""
Created on Sat Dec 22 12:10:34 2018
@author: Metin Mert Akçay && Murat Seymen
"""
...
def trainCNNModel(trainX, trainy):
    ...
    model = Sequential()
    model.add(Conv2D(96, kernel_size=(3, 3), activation='relu', padding='same', input_shape=trainX.shape[1:]))
    ...
    model.fit(trainX, trainy, batch_size=8, epochs=10,callbacks=[checkpointer])
    return model

def testCNNModel(model, testX, testy):
    scores = model.evaluate(x=testX, y=testy, batch_size=8, verbose=1)
...
def draw_confusion_matrix(testX,testy):
    ...
    cm_plot_labels = ['airplanes', 'butterfly', 'car_side', 'cellphone', 'cup', 'dolphin',
                      'headphone', 'laptop', 'Motorbikes', 'pizza']
...
if __name__ == '__main__':
    ...
    # test folder
    path = "Data\\test"
    wanted_class_names = ["airplanes"] # you can add another labels
